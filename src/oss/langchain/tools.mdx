...
:::python
1. **Define** a headless tool with `tool(name=..., description=..., args_schema=...)` from `langchain.tools`, or use `create_headless_tool(...)` directly. In Python, a headless tool is schema-only, with no in-process implementation.
...
In JavaScript, you can share the tool definition between the server and UI, then attach the client-only behavior with `.implement(...)`. In Python, define a schema-only headless tool on the agent, then let the interrupt/resume flow hand execution to your app. For browser-based flows, mirror the schema in the frontend before attaching the client-side behavior.
...
# Define the headless tool with no implementation on the server
save_note = tool(
    name="save_note",
