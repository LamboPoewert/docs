...
LangChain hosts Managed Deep Agents, so you can deploy an agent without running your own infrastructure. Deploying creates or updates the hosted agent resource and syncs the managed file tree of instructions, skills, subagents, and tool configuration. A deploy does not create a [LangSmith Deployment](/langsmith/deployment-quickstart). See [Created resources](/langsmith/managed-deep-agents-overview#created-resources) for what it does create.
...
For a faster path, see the [quickstart](/langsmith/managed-deep-agents-quickstart).
...
The create route, `POST /v1/deepagents/agents`, is shown above under [Create an agent](#create-an-agent).
